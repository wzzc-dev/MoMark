...
MoMark is a practical WYSIWYG Markdown editing demo built on
[MoUI](https://github.com/wzzc-dev/MoUI), extracted from MoUI's
`examples/markdown_editor`. It keeps Markdown source
as the saved value while presenting a formatted editor surface as the primary
screen. It exercises larger document state, rich text input, source-to-visual
mapping, keyboard and contextual commands, and editing workflows.
...
./_build/native/debug/build/wzzc-dev/momark/macos_skia/macos_skia.exe
```
...
```sh
./_build/native/debug/build/wzzc-dev/momark/macos_skia/macos_skia.exe /path/to/document.md
```

The Skia entrypoint requires the same real native Skia link setup used by
MoUI's `examples/showcase/macos_skia`; the MoUI repository's
`scripts/macos-skia-renderer-smoke.sh` can configure those flags temporarily
while running Skia smoke checks. Run
`scripts/macos-skia-renderer-smoke.sh --run-showcase-smoke --run-markdown-smoke`
from a MoUI checkout when you need the renderer pixel smoke plus tester-owned
first-frame Skia smoke logs on macOS.

Windows/Linux Skia and all native WGPU evidence use the canonical Showcase
routes in the MoUI repository. The editor intentionally does not duplicate
those platform/renderer composition roots.
...
When editor work changes the upstream MoUI view wrappers, run the following
from a MoUI checkout:
